<template>
    <div>
        ...
        <nav-bread><span slot="breadName">Cart</span></nav-bread>
                ...
                <li v-for="item in cartList">
                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked}" @click="selectProduct(item)">
                  ...
                  <div class="cart-tab-2">
                    <div class="item-price">{{item.salePrice | formartMoney}}</div>
                  </div>
                  <div class="cart-tab-3">
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="select-self-area">
                          <a class="input-sub" @click="changeNum(item, -1)">-</a>
                          <span class="select-ipt">{{item.productNum}}</span>
                          <a class="input-add" @click="changeNum(item, 1)">+</a>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="cart-tab-4">
                    <div class="item-price-total">{{item.productNum * item.salePrice | formartMoney}}</div>
                  </div>
            ...
            <div class="cart-foot-inner">
              ...
              <div class="cart-foot-r">
                <div class="item-total">
                  总计: <span class="total-price">{{totalMoney | formartMoney}}</span>
                </div>
                <div class="btn-wrap">
                  <a href="/#/address" class="btn btn--red" style="font-size: 15pt;"> 结 算 </a>
                </div>
              </div>
            </div>
          </div>
        ...
        <modal :mdShow="delFlag" @close="closeModal">
            <div slot="message" class="confirm-tips">
                <h2 >你确认删除此订单信息吗?</h2>
            </div>
            <div slot="btnGroup" class="btn-wrap col-2">
              <button class="btn btn--m" id="btnModalConfirm" @click="delProduct">确定</button>
              <button class="btn btn--m btn--red" id="btnModalCancel" @click="closeModal">取消</button>
            </div>
        </modal>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            calculatedMoney(){
                ...
                this.cartList.forEach((item, index)=>{
                    if(typeof item.checked != 'undefined') {
                        item.checked ? this.totalMoney += item.productNum * item.salePrice : null;
                    }else{
            ...
            changeNum(item, oper){
                this.$http.post('/users/changeNum', {
                    ...
                    if(oper > 0){
                        ...
                        item.productNum>0 ? item.productNum-- : null;
                    }
                    this.calculatedMoney();
                });
            },
            selectProduct(item){
                this.$http.post('/users/selectProduct', {
                    productId: item.productId,
                    oper: ''
                }).then(res=>{
