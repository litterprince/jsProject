<template>
    <div>
        ...
        <nav-bread><span slot="breadName">Order Confirm</span></nav-bread>
          ...
          <div class="checkout-order">
            ...
            <div class="price-count-wrap">
              <div class="price-count">
                <ul>
                  <li>
                    <span>商品总计:</span>
                    <span>{{getSubtotal | formartMoney}}</span>
                  </li>
                  <li>
                    <span>运费:</span>
                    <span>{{Shipping | formartMoney}}</span>
                  </li>
                  <li>
                    <span>折扣:</span>
                    <span>{{Discount | formartMoney}}</span>
                  </li>
                  <li>
                    <span>税:</span>
                    <span>{{Tax | formartMoney}}</span>
                  </li>
                  <li class="order-total-price">
                    <span>总 计:</span>
                    <span>{{getTotal | formartMoney}}</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <div class="order-foot-wrap">
              <div class="prev-btn-wrap">
                <a href="/#/address" class="btn btn--m">上一步</a>
              </div>
              <div class="next-btn-wrap">
                <button @click="createOrder" class="btn btn--m btn--red">进行支付</button>
              </div>
            </div>
          </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            ...
            getTotal(){
              return this.getSubtotal + this.Shipping + this.Discount + this.Tax;
            }
        },
        mounted:function () {
            this.getOrderList();
            ...
            getOrderList(){
                this.$http.get('/users/cartList').then(res=>{
                    if(res.data.status == '0' && res.data.result){
                        this.goodsList = res.data.result.filter(item=>item.checked);
                    }
                });
            },
            getAddress(){
                this.$http.get('/users/addressList').then(res=>{
                    if(res.data.status == '0' && res.data.result){
                      res.data.result.forEach((item,index,array)=>{
                          if(item.isDefault) this.address = item;
                      });
                    }
                });
            },
            createOrder(){
                this.$http.post('/users/createOrder',{
                    addressId: this.addressId
                }).then(res=>{
                    if(res.data.status == '0'){
                        this.$router.push({ path: '/orderSuccess?orderId='+res.data.result});
                    }
                });
            }
        }
